...
const createTheatre = async (req,res) => {
    try {
        let response = await theatreService.createTheatre({...req.body, owner: req.user});
        if(response.err){
...
const getMovies = async (req,res) => {
    try {
        let response = await theatreService.getMoviesInATheatre(req.params.theatreId);
    if(response.err){
        errorResponseBody.err = response.err;
        return res.status(response.code).json(errorResponseBody);
    }
    successResponseBody.data = response;
    successResponseBody.message = "successfully fetched all the movies in the theatre"
    return res.status(200).json(successResponseBody);
    } catch (error) {
        console.log(error);
        errorResponseBody.err = error;
...
const checkMovie = async (req,res) => {
    try {
        let response = await theatreService.checkMovieInATheatre(req.params.theatreId,req.params.movieId);
        if(response.err){
            errorResponseBody.err = response.err;
            return res.status(response.code).json(errorResponseBody);
        }
        successResponseBody.data = response;
        successResponseBody.message = "successfully checked if a movie is present in the theatre";
        return res.status(200).json(successResponseBody);
    } catch (error) {
        errorResponseBody.err = error;
        return res.status(500).json(errorResponseBody);
